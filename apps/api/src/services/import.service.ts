		...
		try {
				...
				try {
					...
					if (existing && options.skipExisting) {
						result.warnings.push(
							`Context "${contextData.name}" already exists - skipped`,
						);
						continue;
					}
					...
					result.errors.push(
						`Failed to import context "${contextData.name}": ${message}`,
					);
				}
			}
			...
			for (const termData of data.terms || []) {
				try {
					...
					if (existing && options.skipExisting) {
						result.warnings.push(
							`Term "${termData.name}" already exists - skipped`,
						);
						continue;
					}
					...
					const message =
						error instanceof Error ? error.message : "Unknown error";
					result.errors.push(
						`Failed to import term "${termData.name}": ${message}`,
					);
				}
			}
			...
			for (const tcData of data.termContexts || []) {
				try {
					...
					const context = await contextRepository.findByName(
						tcData.contextName,
					);
					...
					// 既に存在するかを確認
					const exists = await termRepository.existsInContext(
						term[0].id,
						context.id,
					);
			...
			for (const relData of data.relationships || []) {
				try {
					// ソースタームとターゲットタームを検索
					const sourceTerms = await termRepository.searchByName(
						relData.sourceTerm || "",
					);
					const targetTerms = await termRepository.searchByName(
						relData.targetTerm || "",
					);
	...
	private validateImportData(data: unknown): string[] {
		...
		if (Array.isArray(dataObj.contexts)) {
			dataObj.contexts.forEach((context: unknown, index: number) => {
				...
				if (!contextObj.name) {
					errors.push(
						`Context at index ${index} is missing required field: name`,
					);
				}
			});
		}
