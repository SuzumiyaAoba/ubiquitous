...
reviewsRouter.post("/schedule", async (c) => {
	try {
		...
		const message =
			error instanceof Error ? error.message : "Failed to schedule review";
		return c.json(
			{ error: message },
	...
	try {
		...
		const message =
			error instanceof Error
				? error.message
				: "Failed to cancel review schedule";
		return c.json(
			{ error: message },
			error instanceof Error && error.message.includes("not found") ? 404 : 500,
		);
...
reviewsRouter.get("/due", async (c) => {
	try {
		...
		if (asOfDateStr && Number.isNaN(asOfDate?.getTime())) {
			return c.json(
				{ error: "Invalid asOfDate format. Use ISO 8601 format." },
				400,
			);
		}
...
reviewsRouter.post("/", async (c) => {
	try {
		...
		if (!body.termId || !body.reviewedBy || !body.status) {
			return c.json(
				{ error: "termId, reviewedBy, and status are required" },
				400,
			);
		}
		...
		const message =
			error instanceof Error ? error.message : "Failed to execute review";
		return c.json(
			{ error: message },
			error instanceof Error && error.message.includes("not found") ? 404 : 500,
		);
...
reviewsRouter.get("/terms/:termId", async (c) => {
	try {
		...
		const message =
			error instanceof Error ? error.message : "Failed to fetch review history";
		return c.json(
			{ error: message },
			error instanceof Error && error.message.includes("not found") ? 404 : 500,
		);
