...
export const codeElementTypeEnum = pgEnum("code_element_type", [
	"class",
	"method",
	"variable",
]);
...
export const termRelationships = pgTable("term_relationships", {
	...
	sourceTermId: uuid("source_term_id")
		.notNull()
		.references(() => terms.id, { onDelete: "cascade" }),
	targetTermId: uuid("target_term_id")
		.notNull()
		.references(() => terms.id, { onDelete: "cascade" }),
...
export const termsRelations = relations(terms, ({ many }) => ({
	...
	sourceRelationships: many(termRelationships, {
		relationName: "sourceRelationships",
	}),
	targetRelationships: many(termRelationships, {
...
export const termRelationshipsRelations = relations(
	termRelationships,
	({ one }) => ({
...
export const termProposalsRelations = relations(
	termProposals,
	({ one, many }) => ({
		context: one(contexts, {
