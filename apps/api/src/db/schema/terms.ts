...
export const terms = pgTable(
	...
	(table) => ({
		...
		boundedContextIdx: index("idx_terms_bounded_context_id").on(
			table.boundedContextId,
		),
		statusIdx: index("idx_terms_status").on(table.status),
		nextReviewDateIdx: index("idx_terms_next_review_date").on(
		...
		uniqueTermPerContext: unique("unique_term_per_context").on(
			table.name,
			table.boundedContextId,
		),
	}),
