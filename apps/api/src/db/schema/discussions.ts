...
export const discussionThreads = pgTable(
	...
	(table) => ({
		...
		proposalIdIdx: index("idx_discussion_threads_proposal_id").on(
			table.proposalId,
		),
		statusIdx: index("idx_discussion_threads_status").on(table.status),
		createdAtIdx: index("idx_discussion_threads_created_at").on(
			table.createdAt,
		),
		threadReference: check(
			"thread_reference",
			sql`
...
export const comments = pgTable(
	...
	(table) => ({
		threadIdIdx: index("idx_comments_thread_id").on(
			table.threadId,
			table.postedAt,
		),
