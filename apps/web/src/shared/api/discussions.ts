...
import type { DiscussionThread, Comment } from "@ubiquitous/types";
...
export const discussionsApi = {
   ...
   * スレッドを取得
   */
  getThreadById: (id: string) => apiClient.get<DiscussionThread>(`/api/discussions/threads/${id}`),
   ...
   * スレッドを削除
   */
  deleteThread: (id: string) => apiClient.delete<void>(`/api/discussions/threads/${id}`),

  /**
   * コメントを投稿
   */
  createComment: (threadId: string, data: CreateCommentDto) =>
    apiClient.post<Comment>(`/api/discussions/threads/${threadId}/comments`, data),
};
