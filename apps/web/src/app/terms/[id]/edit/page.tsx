...
import { Term, BoundedContext, UpdateTermDto } from '@ubiquitous/types';
import { termsApi, contextsApi } from '@/shared/api';
import { Button, Card, CardHeader, CardTitle, CardContent, Input, Textarea, Select, Loading } from '@/shared/ui';
...
export default function EditTermPage({ params }: { params: { id: string } }) {
  ...
  const [formData, setFormData] = useState<UpdateTermDto & { boundedContextId?: string; examples?: string[] }>({
    name: '',
    definition: '',
    boundedContextId: '',
    examples: [''],
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          examples: termData.examples && termData.examples.length > 0 ? termData.examples : [''],
  ...
  return (
    <div className="max-w-4xl mx-auto p-8">
      <div className="mb-6">
        <Link href={`/terms/${params.id}`} className="text-blue-600 hover:text-blue-800 text-sm">
          ← 詳細に戻る
        </Link>
      </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">
                  {contexts.find(c => c.id === formData.boundedContextId)?.name || '不明'}
                </p>
            ...
            <div className="space-y-4">
              <div className="flex items-center">
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, essentialForOnboarding: e.target.checked })
                  }
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                />
                <label htmlFor="essentialForOnboarding" className="ml-2 text-sm text-gray-700">
                  オンボーディングに必須
                </label>
              </div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    reviewCycleDays: e.target.value ? parseInt(e.target.value) : undefined,
                  })
                }
                min={1}
              />
